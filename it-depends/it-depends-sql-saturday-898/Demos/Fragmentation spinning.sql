...
BEGIN 
	USE master 
	DROP DATABASE IF EXISTS spinning;
	CREATE DATABASE spinning
	 CONTAINMENT = NONE
	 ON  PRIMARY 
	( NAME = N'spinning', FILENAME = N'd:\sqldata\MSSQL13.MSSQLSERVER\MSSQL\Data\spinning.mdf' , SIZE = 1024000KB , FILEGROWTH = 512000KB )
	 LOG ON 
	( NAME = N'spinning_log', FILENAME = N'd:\sqldata\MSSQL13.MSSQLSERVER\MSSQL\Data\spinning_log.ldf' , SIZE = 512000KB , FILEGROWTH = 512000KB )
	--Stolen with pride from Tibor Karaszi
	--www.karaszi.com
END 
GO
	SET NOCOUNT ON
	ALTER EVENT SESSION frag_test_spinning ON SERVER STATE = STOP
	WAITFOR DELAY '00:00:02'
	--Delete XE file, using xp_cmdshell (bad, I know)

	EXEC sp_configure 'xp_cmdshell', 1 RECONFIGURE WITH OVERRIDE
	EXEC xp_cmdshell 'DEL D:\SQLDATA\frag_test_spinning*.xel', no_output
	EXEC sp_configure 'xp_cmdshell', 0 RECONFIGURE WITH OVERRIDE
	IF EXISTS (SELECT * FROM sys.server_event_sessions WHERE name = 'frag_test_spinning')
	DROP EVENT SESSION frag_test_spinning ON SERVER
	CREATE EVENT SESSION frag_test_spinning ON SERVER
	ADD EVENT sqlserver.sp_statement_completed()
	ADD TARGET package0.event_file(SET filename=N'd:\sqldata\frag_test_spinning')
...
ALTER EVENT SESSION frag_test_spinning ON SERVER STATE = START
...
SELECT TOP(1000*1000) ROW_NUMBER() OVER( ORDER BY (SELECT NULL)) AS c1, CAST('Hello' AS char(8)) AS c2
INTO narrow_index
...
ALTER INDEX cl_wide ON wide REBUILD WITH(FILLFACTOR = 52)
--Run the queries
...
INTO myXeData
FROM sys.fn_xe_file_target_read_file('d:\sqldata\frag_test_spinning*.xel', NULL, NULL, NULL);
...
--SELECT * INTO PerformanceResults.dbo.SpinningRaw FROM myXeData2 ORDER BY index_width, cold_or_warm, fragmented
...
--INTO PerformanceResults.dbo.SpinningAvg
